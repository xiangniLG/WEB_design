...
def download(url,mycursor,db):
  ...
  for movie_detail_link in movie_detail_links:
    [rating,director,actor]=download_moviedetail(movie_detail_link)
  ...
  for detaillink , name, link,rating,director,actor in zip(movie_detail_links, pic_name_list, pic_link_list,ratings,directors,actors):
    sql="insert into moviedetails(movie_link,movie_name,movie_pic_link,movie_rating,movie_director,movie_actor1,movie_actor2,movie_actor3)\
         values(%s,%s,%s,%s,%s,%s,%s,%s)"
    values=(detaillink , name, link,rating,director,actor[0],actor[1],actor[2])
...
# download(start_urls[9],mycursor,db)
for url in start_urls:
  download(url,mycursor,db)
